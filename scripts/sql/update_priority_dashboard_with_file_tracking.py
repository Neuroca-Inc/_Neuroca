#!/usr/bin/env python3
"""
Update priority_dashboard to include real-time file tracking metrics
"""
...
def update_dashboard():
    """Update priority_dashboard to include file tracking metrics."""
    
    print("🔄 UPDATING PRIORITY DASHBOARD WITH FILE TRACKING")
    print("=" * 60)
    ...
    # Create enhanced priority_dashboard with file tracking
    enhanced_dashboard = """
    ...
    SELECT 
        ...
        (SELECT COUNT(DISTINCT component_id) FROM file_activity_log WHERE timestamp > datetime('now', '-7 days')) as active_components_week,
        ...
        ' changes, ' ||
        (SELECT COUNT(DISTINCT component_id) FROM file_activity_log WHERE timestamp > datetime('now', '-24 hours')) ||
        ' components' as activity_summary

    FROM (
        SELECT 
            AVG(CASE WHEN c.status = 'Completed' THEN 100.0 
                     WHEN c.status = 'In Progress' THEN 75.0 
                     WHEN c.status = 'Testing' THEN 85.0
                     WHEN c.status = 'Planned' THEN 25.0
    ...
    SELECT 
        ...
        CASE 
            ...
            WHEN c.status = 'In Progress' THEN '🔄 ACTIVE'
            WHEN c.status = 'Testing' THEN '🧪 TESTING'
            WHEN c.status = 'Planned' THEN '📋 PLANNED'
            ELSE '❓ UNKNOWN'
        END as status,
        CASE WHEN c.status = 'Completed' THEN 100.0 
             WHEN c.status = 'In Progress' THEN 75.0 
             WHEN c.status = 'Testing' THEN 85.0
             WHEN c.status = 'Planned' THEN 25.0
        ...
        COALESCE(c.last_updated, c.created_at, datetime('now')) as last_updated,
