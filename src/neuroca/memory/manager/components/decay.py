...
class MemoryManagerDecayMixin:
    """Expose APIs for decaying stored memories."""
    ...
    async def decay_memory(
        ...
        """Decay a memory to reduce its prominence."""
        ...
        try:
            ...
            LOGGER.exception("Failed to decay memory %s", memory_id)
            raise MemoryManagerOperationError(
                f"Failed to decay memory: {exc}"
            ) from exc


__all__ = ["MemoryManagerDecayMixin"]
